...
fn fetch_single_valid_doi() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stdout.contains("10.1000/182"),
        "stdout did not contain expected DOI. stdout=\n{}",
        stdout
    );
    assert!(
...
fn fetch_non_existent_doi() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stdout.is_empty(),
        "stdout should be empty for non-existent DOI, got=\n{}",
        stdout
    );
    assert!(
...
fn fetch_invalid_identifier() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        stdout.is_empty(),
        "stdout should be empty for invalid identifier, got=\n{}",
        stdout
    );
    assert!(
